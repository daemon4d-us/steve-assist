...
- `services/call_control.rs` — the `end_call` tool. It is always offered next to the calendar tools; when the model calls it, the stream handler lets the farewell play out, closes the media stream (which ends a `<Connect><Stream>` call and makes the Bluetooth bridge hang up) and, for Twilio call sids, also completes the call via the REST API. The system prompt (both builders) tells the model to use it once a conversation is over, and includes the current date/time in the profile's time zone. Because Nemotron sometimes ends a call in words instead of calling the tool, the handler also hangs up on a verbal hang-up marker in the reply (stripped before TTS), on two consecutive farewell turns, and when a call exceeds `MAX_CALL_SECONDS` (default 900)
...
- A `start` event whose custom parameters carry `objective` (with `contactPhone`, optional `contactName`/`profileId`) is treated as an outbound call exactly like a scheduler assignment; the Bluetooth bridge uses this for calls the phone places (`steve-bridge dial`) or that the user dialed with an objective file set
